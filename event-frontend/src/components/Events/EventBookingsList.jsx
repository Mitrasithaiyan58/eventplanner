...
const EventDashboard = () => {
  ...
  // load manager from storage and fetch dashboard data
  useEffect(() => {
  ...
  const handleVendorBookingStatus = async (id, status) => {
    try {
      // backend endpoint: PUT /vendor-booking/{id}/confirm or /reject (ours only has /confirm; reject not implemented)
  ...
  return (
    <div className="manager-dashboard">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div>
          <button onClick={() => navigate("/ai-suggestions")} style={{ marginRight: 8 }}>💡 AI Event Names</button>
          <button onClick={() => navigate("/create-event?ai=true")} style={{ marginRight: 8 }}>✨ Create Event with AI</button>
      ...
      <div className="dashboard-tabs" style={{ marginTop: 16 }}>
        ...
        <button className={activeTab === "managerEvents" ? "active" : ""} onClick={() => setActiveTab("managerEvents")}>📋 My Events</button>
      ...
      {activeTab === "bookings" && (
        <div className="tab-content" style={{ marginTop: 12 }}>
          ...
          {!fetchError && bookings.length === 0 ? <p>No bookings yet.</p> : bookings.map((b) => (
      ...
      {activeTab === "inquiries" && (
        <div className="tab-content" style={{ marginTop: 12 }}>
          {fetchError && <p style={{ color: "red" }}>{fetchError}</p>}
          {!fetchError && inquiries.length === 0 ? <p>No inquiries yet.</p> : inquiries.map((q) => (
      ...
      {activeTab === "managerEvents" && (
        <div className="tab-content" style={{ marginTop: 12 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 13, color: "#666" }}>
                      {ev.eventDateTime ? new Date(ev.eventDateTime).toLocaleString() : "-"} • {ev.location || "N/A"}
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div className={`status-badge ${ev.status?.toLowerCase() || "planned"}`}>{ev.status || "Planned"}</div>
                  </div>
                </div>
              </div>
            ))
          ...
          {fetchError && <p style={{ color: "red" }}>{fetchError}</p>}
          {!fetchError && vendorBookings.length === 0 ? <p>No vendor bookings yet.</p> : vendorBookings.map((b) => (
            <div key={b.id} className="notif-item">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
